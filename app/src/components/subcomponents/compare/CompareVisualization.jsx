...
class CompareVisualization extends Component {
  ...
  genGraphData(data, layer, model) {
    ...
    this.layerScores[model].data.push({
      layer: layer.name,
      index: layer.layer_index,
      score: weightedScore * 1,
    });

    // check if we have all layers for current model
    if (this.layerScores[model].data.length == this.props.data.numLayers) {
      this.layerScores[model].data = _.sortBy(
        this.layerScores[model].data,
        "index"
      );
      let maxVal = _.maxBy(this.layerScores[model].data, "score");

      this.layerScores[model].maxindex = this.layerScores[model].data.indexOf(
        maxVal
      );
      ...
      if (Object.keys(this.layerScores).length == this.props.data.numModels) {
        ...
        let backStrokes = document.getElementsByClassName(
          "backstroke" + this.overallBestScore
        );
        let textBoxes = document.getElementsByClassName(
          "topmodel" + this.overallBestScore
        );
      ...
      layers.forEach((layer) => {
        ...
        loadedJSON.then(function (data) {
          if (data) {
            ...
            self.genGraphData(
              data[self.props.data.selectedimage + ""],
              layer,
              model.name
            );
          }
  ...
  drawChart(layerScores) {
    ...
    var svg = d3
      .select("div.comparevisualizationbox")
      .append("svg")
    ...
    // add the y Axis
    let yAxis = d3.axisRight(y).tickSize(this.miniChartWidth);
    ...
    svg
      ...
      .attr("class", function () {
        return (
          "boldtext displaynone overallbest smalldesc topmodel" +
          layerScores.maxvalue
        );
      })
      .text("*BEST");
    ...
    svg
      ...
      .attr(
        "x",
        0 - self.miniChartHeight / 2 + (margin.top + margin.bottom) / 2
      )
      .attr("dy", "2em")
      .style("text-anchor", "middle")
      .attr("class", "smalldesc")
    ...
    function customYAxis(g) {
      ...
      g.selectAll(".tick:not(:first-of-type) line")
        ...
        .attr("class", " backstroke" + layerScores.maxvalue);
  ...
  render() {
    let selectedImagePath =
      ...
      this.props.data.dataset +
      "/" +
      this.props.data.selectedimage +
      ".jpg";
    let bestModelsList = this.state.bestModels.map((best, index) => {
      // console.log(best)
      return (
        <div key={"bestmodel" + index} className="iblock mr5 greentext">
          {" "}
          {best.model.toUpperCase() + "[" + best.score + "]"},{" "}
        </div>
      );
    });
    return (
      <div>
        <div className="flex mb10">
          ...
          <div className="flexfull">
            For the current search query (image shown on the left), the charts
            below allows you to <strong>compare search score </strong> results
            obtained when we use layers from all{" "}
            <strong> {this.props.data.numModels} </strong> models. The{" "}
            <span className="greentext boldtext"> green bar </span>
        ...
        <div className="comparevisualizationbox">
          {this.state.loadingCompare && (
            <InlineLoading description="Comparing scores for all models .."></InlineLoading>
          )}
        </div>
