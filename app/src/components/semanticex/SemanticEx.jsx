...
import CompareVisualization from "../comparevisualization/CompareVisualization";
import CompareAllVisualization from "../comparevisualization/CompareVisualization";
...
class SemanticEx extends Component {
  ...
  getNextVisible(selectedModel) {
    let modelVisible = checkInView(
      this.refs["modelscrollbox"],
      this.refs["modelimg" + selectedModel],
      true,
      containerOffset,
      elementOffset
    );
  ...
  getNextVisibleLayer(selectedLayer) {
    ...
    let layerVisible = checkInView(
      this.refs["layerscrollbox"],
      this.refs["layerimg" + selectedLayer],
      true,
      containerOffset,
      elementOffset
    );
    if (layerVisible) {
      ...
      let nextIndex =
        (selectedLayer * 1 + 1) %
        this.state.modelsList[this.state.selectedmodel].layers.length;
  ...
  drawLines(e) {
    if (this.state.showmodelconfig) {
      ...
      let modelVisible = checkInView(
        self.refs["modelscrollbox"],
        self.refs["modelimg" + this.state.selectedmodel],
        true,
        containerOffset,
        elementOffset
      );
      ...
      layers.forEach(function (each, i) {
        let layerVisible = checkInView(
          self.refs["layerscrollbox"],
          self.refs["layerimg" + i],
          true,
          containerOffset,
          elementOffset
        );
        if (layerVisible && modelVisible) {
          ...
          let line = new LeaderLine(
            self.refs["modelimg" + self.state.selectedmodel],
            self.refs["layerimg" + i],
            {
              ...
              endPlugSize:
                maxLineWidth / Math.min(minLineWidth + i * incs, maxLineWidth),
            }
          );
  ...
  cycleLayerModel(val) {
    if (this.lastclicked == "model") {
      let newState = this.getNextVal(
        this.state.selectedmodel * 1 + val,
        this.state.modelsList.length
      ); // Math.max((this.state.selectedmodel*1 + val) % this.state.modelsList.length, 0)
      ...
      let newState = this.getNextVal(
        this.state.selecteddataset * 1 + val,
        this.state.datasetsList.length
      );
      if (!isNaN(newState)) {
      ...
      let newState = this.getNextVal(
        this.state.selectedlayer * 1 + val,
        this.state.modelsList[this.state.selectedmodel].layers.length
      );
      if (!isNaN(newState)) {
      ...
      let numNeurons = this.layerList[
        this.state.modelsList[this.state.selectedmodel].name
      ][
        this.state.modelsList[this.state.selectedmodel].layers[
          this.state.selectedlayer
        ].name
      ].length;
      let newState = this.getNextVal(
        this.state.selectedneuron * 1 + val,
        numNeurons
      );
  ...
  componentDidMount() {
    ...
    this.refs["layerscrollbox"].addEventListener(
      "scroll",
      this.scrollEndedHandler,
      false
    );
    this.refs["modelscrollbox"].addEventListener(
      "scroll",
      this.scrollEndedHandler,
      false
    );
  ...
  componentWillUnmount() {
    ...
    this.refs["layerscrollbox"].removeEventListener(
      "scroll",
      this.scrollEndedHandler,
      false
    );
    this.refs["modelscrollbox"].removeEventListener(
      "scroll",
      this.scrollEndedHandler,
      false
    );
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      ...
      this.state.selectedlayer !== prevState.selectedlayer ||
      this.state.selecteddataset !== prevState.selecteddataset
    ) {
      this.updateSimilarity();
      this.showTopResults();
      this.searchCount++;
  ...
  clickDatasetImage(e) {
    registerGAEvent(
      "semanticsearch",
      "datasetchange",
      this.state.datasetsList[e.target.getAttribute("indexvalue")].name,
  ...
  clickModelImage(e) {
    registerGAEvent(
      "semanticsearch",
      "modelchange",
      this.state.modelsList[e.target.getAttribute("indexvalue")].name,
      this.componentLoadedTime
    );
  ...
  clickLayerImage(e) {
    registerGAEvent(
      "semanticsearch",
      "layerchange",
      this.state.datasetsList[this.state.selecteddataset].name +
        "," +
        this.state.modelsList[this.state.selectedmodel].layers[
          e.target.getAttribute("indexvalue")
        ].name,
      this.componentLoadedTime
    );
  ...
  clickMetricImage(e) {
    registerGAEvent(
      "semanticsearch",
      "metricchange",
      this.state.distanceMetricList[e.target.getAttribute("indexvalue")],
      this.componentLoadedTime
    );
  ...
  toggleSemanticModal(e) {
    registerGAEvent(
      "semanticsearch",
      "semanticmodal",
      this.state.showorientationmodal ? "close" : "open",
      this.componentLoadedTime
    );
  ...
  toggleUMAPView(e) {
    registerGAEvent(
      "semanticsearch",
      "umapviewe",
      this.state.showumap ? "close" : "open",
      this.componentLoadedTime
    );
  ...
  toggleShowCompare(e) {
    registerGAEvent(
      "semanticsearch",
      "comparemodels",
      this.state.showcomparemodal ? "off" : "on",
      this.componentLoadedTime
    );
  ...
  toggleAllCompareView() {
    registerGAEvent(
      "semanticsearch",
      "allcompare",
      this.state.showcompareallmodels ? "close" : "open",
      this.componentLoadedTime
    );
  ...
  updateSimilarity() {
    let similarityPath =
      ...
      this.state.modelsList[this.state.selectedmodel].name +
      "/" +
      this.state.distanceMetricList[this.state.selectedmetric] +
      "/" +
      this.state.modelsList[this.state.selectedmodel].layers[
        this.state.selectedlayer
      ].name +
  ...
  render() {
    ...
    let datasetImageList = this.state.datasetsList.map((dsdata, index) => {
      let iconPath =
        process.env.PUBLIC_URL + "/assets/semsearch/images/" + dsdata.icon;
      // console.log(dsdata)
      return (
        <div
        ...
        >
          ...
          <img
            onClick={this.clickDatasetImage.bind(this)}
            src={iconPath}
            alt=""
            className={
              "datasetbox rad2 " +
              (this.state.selecteddataset == index ? "active" : "")
            }
            indexvalue={index}
          />
        </div>
    ...
    let modelImageList = this.state.modelsList.map((mdata, index) => {
      ...
      let selectedlayer = this.state.modelsList[index].layers[
        this.state.modelsList[index].layers.length - 1
      ].name;
      ...
      return (
        ...
        >
          ...
          <img
            ref={"modelimg" + index}
            onClick={this.clickModelImage.bind(this)}
            src={imagePath}
            alt=""
            className={
              "datasetbox rad2 " +
              (this.state.selectedmodel == index ? "active" : "")
            }
            indexvalue={index}
          />
        </div>
    ...
    let layerImageList = this.state.modelsList[
      this.state.selectedmodel
    ].layers.map((ldata, index) => {
      let selectedModel = this.state.modelsList[this.state.selectedmodel].name;
      let selectedlayer = this.state.modelsList[this.state.selectedmodel]
        .layers[index].name;
      ...
      return (
        ...
        >
          ...
          <img
            ref={"layerimg" + index}
            onClick={this.clickLayerImage.bind(this)}
            src={imagePath}
            alt=""
            className={
              "datasetbox rad2 " +
              (this.state.selectedlayer == index ? "active" : "")
            }
            indexvalue={index}
          />
        </div>
    ...
    let metricImageList = this.state.distanceMetricList.map((metric, index) => {
      return (
        ...
        >
          ...
          <img
            onClick={this.clickMetricImage.bind(this)}
            src={require("../../images/bgwhite.png")}
            alt=""
            className={
              "datasetbox rad2 " +
              (this.state.selectedmetric == index ? "active" : "")
            }
            indexvalue={index}
          />
        </div>
    ...
    let datasetClasses = this.datasetdictionary["classlist"][
      this.state.datasetsList[this.state.selecteddataset].name
    ];

    let datasetClassImagesList = [];
    let co = datasetClasses.map((className, index) => {
      let classcon = this.datasetdictionary["classes"][
        this.state.datasetsList[this.state.selecteddataset].name
      ];
          ...
          >
            <img
              ...
              className={
                ...
                (this.state.selectedsimimage == classval ? "active" : "")
              }
              indexvalue={classval}
            />
          </div>
      ...
      let header = (
        <div
          ...
          className="iblock mr5 categorymain  mb5 "
        >
          <div>
    ...
    let similarImagesList = this.state.similarityArray[
      this.state.selectedsimimage
    ].map((alldata, index) => {
      let imagePath =
        process.env.PUBLIC_URL +
        "/assets/semsearch/datasets/" +
    ...
    let simArr = this.state.similarityArray[this.state.selectedsimimage].slice(
      1,
      this.state.topx + 1
    );
    let allDictionary = this.datasetdictionary.dictionary[
      this.state.datasetsList[this.state.selecteddataset].name
    ];
      ...
      let returnValue = (
        ...
        >
          <img
            ...
            className={
              "simiimage clickable rad2 " +
              (this.state.selectedsimimage == alldata[0] ? "active" : "")
            }
            indexvalue={alldata[0]}
          />
        </div>
      ...
      <div className="mainsemanticdiv">
...

        {
          <Modal
            ...
            modalHeading={this.state.datasetsList[
        ...
        <div className="flex ">
          <div className="flex5 ">
            <div className="mynotif h100 lh10 pl  instructions lightbluehightlight maxh16 mr10">
              ...
              <strong> semantic image search </strong> using convolutional
              neural networks
              <span className="">
                {" "}
                [
              ...
              <strong> top {this.state.topx} </strong> most similar images to
              the selected image.
            </div>
          </div>
        ...
        <div className={" " + (this.state.showadvanced ? "" : " displaynone")}>
          ...
          <div
            onClick={this.toggleModelConfig.bind(this)}
          ...
          >
            <div className="iblock flexfull minwidth485">
              ...
              <strong>
                {" "}
                {!this.state.showmodelconfig && <span>&#x25BC; </span>}{" "}
                {this.state.showmodelconfig && <span>&#x25B2; </span>}{" "}
              </strong>{" "}
          ...
          {/* Configuration drawer */}
          {
            <div
              ...
              className={
                ...
                (this.state.showmodelconfig ? "" : " displaynone")
              }
            >
                ...
                <div className="pb8 sectiontitle ">
                  ...
                  <div className="iblock ">
                    <Tooltip direction="left" triggerText="">
                      <div className="wscore">
                        We construct{" "}
                        <span className="italics">intermediate models</span>{" "}
                        from the main model at each of these layers. Each
                        intermediate model has less parameters than the full
                        model and is used for feature extraction.
                  ...
                  <div className="flexfull ">
                    ...
                    <div className="smalldesc pt4">
                      ...
                      <strong>
                        ...
                        {
                          this.state.modelsList[this.state.selectedmodel]
                            .layers[this.state.selectedlayer].type
                        }{" "}
                      </strong>{" "}
                      |{" "}
          ...
          <div
            style={{ zIndex: 100 }}
            onClick={this.toggleAllCompareView.bind(this)}
            className="unselectable mt10 p10 clickable  flex greymoreinfo"
          >
            <div className="iblock flexfull minwidth485">
              {" "}
              <strong>
                ...
                {this.state.showcompareallmodels && <span>&#x25B2; </span>}{" "}
              </strong>{" "}
              Compare Models{" "}
            </div>
            <div className="iblock   ">
            ...
            <div className="modelconfigdiv comparediv p10">
              <CompareAllVisualization
                data={{
                  ...
                  dataset: this.state.datasetsList[this.state.selecteddataset]
                    .name,
                  layers: this.state.modelsList[this.state.selectedmodel]
                    .layers,
                  chartWidth: 290,
                  chartHeight: 220,
          ...
          <div
            style={{ zIndex: 100 }}
            onClick={this.toggleUMAPView.bind(this)}
            className="unselectable mt10 p10 clickable  flex greymoreinfo"
          >
            <div className="iblock flexfull minwidth485">
              {" "}
            ...
            <div className="iblock   ">
              ...
              <div className="iblock">
                <div className="smalldesc">
                  ...
                  / {this.state.modelsList[this.state.selectedmodel].numlayers}{" "}
                </div>
              </div>
            </div>
          </div>
        ...
        {(this.state.showtopresults || this.searchCount > 0) && (
          <div>
            <div className="unselectable mt10    flex ">
              <div
                onClick={this.toggleTopX.bind(this)}
                className=" iblock clickable greymoreinfo flexfull minwidth485 p10"
              >
                {" "}
                <strong>
                  {" "}
                  {!this.state.showtopresults && <span>&#x25BC; </span>}{" "}
                  {this.state.showtopresults && <span>&#x25B2; </span>}{" "}
                </strong>
                <strong>Top {this.state.topx} results </strong> based on your
                search configuration
                <span className="smalldesc">
                  {" "}
                  [<strong> MODEL: </strong>{" "}
...

        {
          <div ref="topresultsbox" className="sliderboxcontainer transition3s">
            <div
              className={
                " sliderbox topconfig" +
                (this.state.showtopresults ? " open" : " closed")
              }
            >
                      ...
                      <div className="topscorediv">
                        ...
                        <div className="weightedscore smalldesc textaligncenter">
                          <Tooltip triggerText="What is this?">
                            <div className="wscore">
                              This is the percentage of returned results that
                              belong to the same category as the selected image
                              (weighted by position in the result list). For the
                              current search,{" "}
                              <strong>
                                {simCount} / {this.state.topx} results{" "}
                              </strong>{" "}
                              are in same category{" "}
                              <strong>({selectedCat.toUpperCase()})</strong>.
                              Note that this score is conservative - some images
                              may belong to different classes but are{" "}
                              <span className="italics"> similar </span> (e.g
                              sedan, beetle, ferrari are{" "}
        ...
        {this.state.showtopresults && (
          <div className="mb5 pt5 mediumdesc lhmedium textalignright mr10">
            Search results not awesome? <strong>Hint:</strong> Try a different
            model or layer.
          </div>
        )}
        ...
        <div className="">
          ...
          <div className="mb10 mt10 ">
            ...
            <div
              onClick={this.toggleViewDatasetBy.bind(this)}
              className={
                "p10 greytab greyhighlight clickable unselectable greymoreinfo iblock mr5 " +
                (this.state.viewdatasetby == "all" ? "active" : "")
              }
              viewby="all"
            >
              {" "}
            ...
            <div
              onClick={this.toggleViewDatasetBy.bind(this)}
              className={
                "p10 greytab greyhighlight clickable unselectable greymoreinfo iblock mr10 " +
                (this.state.viewdatasetby == "category" ? "active" : "")
              }
              viewby="category"
            >
              {" "}
            ...
            <div className="boldtext sectiontitle  iblock   h100 mt10  mr10">
              {" "}
              DATASET: [{" "}
              {this.state.datasetsList[
                this.state.selecteddataset
            ...
            <div className="iblock pt10">
              {" "}
              {
                this.state.datasetsList[this.state.selecteddataset].description
              }{" "}
            </div>
          </div>
          {/* <div className="horrule mb10"></div> */}
          ...
          <div className="lightbluehightlight mb10 ">
            {" "}
